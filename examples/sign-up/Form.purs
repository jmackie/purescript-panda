  ...
  PH.form
    ...
    [ PH.label
        ...
        , PH.input
            ...
            , PP.required true
            , PP.type_ "text"
            , modifyClassWhen "SignUp__email" \{ input } -> case input of
                ShowError UnknownEmail -> Just "error"
                _ -> Nothing
            ]
        , fieldError \{ input, state } -> case input of
            ShowError UnknownEmail -> Just "Hmm, are you sure we've met?"
            _ -> Nothing
        ]
    , PH.label
        [ PP.className "SignUp__label"
        ...
        , PH.input
            ...
            , PP.required true
            , PP.type_ "password"
            , modifyClassWhen "SignUp__password" \{ input } -> case input of
                ShowError UnknownEmail -> Just "error"
                _ -> Nothing
            ]
        , fieldError \{ input, state } -> case input of
            ShowError InvalidPassword -> Just "That's not how I remember it..."
            _ -> Nothing
        ]
    , PH.input
        ...
        , PP.watch \{ input } -> case input of
            ...
            _ -> PP.Ignore
        , PP.watch \{ input } -> case input of
            SetLoadingState true -> PP.SetTo (PP.disabled true)
            SetLoadingState false -> PP.Clear
            _ -> PP.Ignore
        ]
    ]
...
modifyClassWhen baseName check =
  PP.watch \instruction ->
    PP.SetTo
      $ PP.className case check instruction of
